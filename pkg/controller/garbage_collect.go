...
func (c *MessengerController) garbageCollect(stopCh <-chan struct{}, gcTime time.Duration) {
	for {
		...
		messages, err := c.messengerClient.MessengerV1alpha1().Messages(metav1.NamespaceAll).List(metav1.ListOptions{})
		if err == nil {
			...
			go func() {
				for _, msg := range messages.Items {
					...
					if age.Before(time.Now()) {
						c.messengerClient.MessengerV1alpha1().Messages(msg.Namespace).Delete(msg.Name, deleteInBackground())
					}
				}
				doneC <- true
			}()
