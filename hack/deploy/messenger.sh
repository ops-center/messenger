...
if [ "$APPSCODE_ENV" = "dev" ]; then
  ...
  export MESSENGER_IMAGE_PULL_POLICY=Always
fi
...
if [ "$MESSENGER_ENABLE_VALIDATING_WEBHOOK" = true ]; then
  ...
  $ONESSL wait-until-ready apiservice v1alpha1.admission.messenger.appscode.com || {
...
for crd in "${crds[@]}"; do
  $ONESSL wait-until-ready crd ${crd}.messenger.appscode.com || {
